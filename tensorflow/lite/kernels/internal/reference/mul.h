...
inline void BroadcastMul6DSlow(const ArithmeticParams& params,
  ...
  BroadcastMulRecursiveDimensions(
      ...
      [](const ArithmeticParams& lambda_params, const uint8_t input1_val,
         const uint8_t input2_val) {
        const int32_t offsetted_input1_val = lambda_params.input1_offset + input1_val;
        const int32_t offsetted_input2_val = lambda_params.input2_offset + input2_val;
        const int32_t unclamped_result =
            lambda_params.output_offset +
            MultiplyByQuantizedMultiplier(
                offsetted_input1_val * offsetted_input2_val,
                lambda_params.output_multiplier, lambda_params.output_shift);
        const int32_t clamped_output = std::min(
            lambda_params.quantized_activation_max,
            std::max(lambda_params.quantized_activation_min, unclamped_result));
...
BroadcastMul6DSlow(const ArithmeticParams& params,
  ...
  BroadcastMulRecursiveDimensions(
      params, 0, input1_data, input2_data, output_data, &input1_offset,
      &input2_offset, &output_offset, desc1, desc2, extended_output_shape_dims,
      [](const ArithmeticParams& lambda_params, const T input1_val,
         const T input2_val) {
        T output_activation_min;
        T output_activation_max;
        GetActivationParams(lambda_params, &output_activation_min,
                            &output_activation_max);
...
inline void BroadcastMul6DSlow(const ArithmeticParams& params,
  ...
  BroadcastMulRecursiveDimensions(
      params, 0, input1_data, input2_data, output_data, &input1_offset,
      &input2_offset, &output_offset, desc1, desc2, extended_output_shape_dims,
      [](const ArithmeticParams& lambda_params, const std::complex<float> input1_val,
